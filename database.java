...
public class database {
    ...
    static boolean checkenroll(String n) throws Exception {
        ...
        String enrollmentNumberToCheck = n; 
...
frame.setBounds(300, 300, 500, 500);
    ...
    static boolean check(String enrol){
        try {
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/student", "root", "123456");
            PreparedStatement ps2 = con.prepareStatement("DELETE FROM CHECK_TABLE WHERE ENROLL = ?");
            String enroll = enrol;
        ...
        try {
         ...
         PreparedStatement ps = con.prepareStatement("DELETE FROM INFO WHERE ENROLL = ?");
            String enroll = t1.getText();
             ps.setString(1,enroll);
                int n =  ps.executeUpdate(); 
           boolean r = check(enroll);
          if(n==1 && r){
            JOptionPane.showMessageDialog(null, "Enrollment and password are removed "+n+" ");
            System.out.println(" "+n);
          }else{
            JOptionPane.showMessageDialog(null, " and password are already removed"+n+" ");
            System.out.println(" "+n);
          }
        } catch (Exception ex) {
           System.out.println(ex);
           JOptionPane.showMessageDialog(null, "Enrollment and  are already removed");
